...
function toggleDarkMode(
    ...
    function doTheThing() {
        ...
        link.href = dark_mode_is_currently
            ? ""
            : "/static/css/darkmode/darkmode-" +
              (mobileRequest ? "mobile" : "desktop") +
              ".css";
        Array.from(
            document.getElementsByClassName("theme-color-setting")
        ).forEach((e) => {
            e.setAttribute(
                "content",
                dark_mode_is_currently ? "#4343b2" : "#373737"
            );
        });
...
function checkIfIDTextFits() {
    ...
    if (
        el.val() != lastKnownID ||
        inputExceeded() ||
        screenSize != lastKnownScreenSize
    ) {
        if (inputExceeded()) {
            while (inputExceeded()) {
...
function showSaved() {
    ...
    $(".savedList").empty();
    $(".savedList").append(
        "<li class='savedItems'>Tryck på länkarna för att ta bort dom</li>"
    );
    ...
    for (var i = savedItems.length - 1; i >= 0; i--) {
        if (savedItems[i].length > 0) {
            $(".savedList").append(
                '<li class="savedItems" onclick="eraseCookie(' +
                    "'" +
                    savedItems[i] +
                    "'" +
                    ');showSaved();">' +
                    savedItems[i].replace("URL_", "") +
                    "</li>"
            );
        }
    }
    ...
    if ($(".savedItems").length == 1) {
        $(".savedList").empty();
        $(".savedList").append(
            "<li class='savedItems'>Du har inte sparat något URL</li>"
        );
    } else {
        $(".savedList").append(
            '<button class="clearSavedItems mobileSaved control-container" onclick="deleteAllURLCookies();">Ta bort alla</button>'
        );
    }
...
function updateMenuButtonsBasedOnSize() {
    ...
    $(".menu-option-text").attr(
        window.innerWidth < 450 ? "shortText" : "longText",
        function (i, x) {
            if (x != undefined) {
                t[i].innerHTML = x;
            }
...
function clickOn_QRCODE() {
    ...
    try {
        window.location.href =
            ...
            "API/QR_CODE?id=" +
            (privateURL
                ? getShareableURL()["id"] + "&p=1"
                : $("#id-input-box").val() + "&p=0");
    } catch {
...
function update_dropdowns() {
    ...
    if (mobileRequest) {
        console.log(
            "update_dropdowns() was stopped (this is mobile request, we dont need it)"
        );
        return;
    }
...
function install() {
    ...
    window.deferredPrompt.userChoice.then((choice) => {
        ...
        if (choice.outcome == "accepted") {
            $.getJSON(
                window.location.pathname + "static/manifest.webmanifest",
                function (data) {
...
function update_timetable_to_fit_new_window_size_function(
    do_updateTimetable = true
) {
...
$(window).on("load", function () {
    ...
    $("#id-input-box").val(initID == "" ? readCookie("idnumber") : initID);
    checkIfIDTextFits();
...
$(document).ready(function () {
    ...
    // }
    if (mobileRequest) {
        $(".input-idnumber").width(
            `calc(calc(100vw - 17px) - ${$(".arrows-container").width()}px)`
        );
    }
